import logo64 from '../assets/cricket-catapult.png'
import { Link } from 'react-router-dom'
import PropsType from 'prop-types'

function Navbar({ title }) {
    return <nav className='navbar shadow-lg bg-neutral text-neutral-content'>
        <div className='container mx-auto'>
            <div className='flex px-2 mx-2 space-x-4'>
                <img className='pt-1 h-10 w-10' alt='' src={logo64} />
                <Link to='/' className='text-2xl pt-1 font-bold align-middle'>
                    Catapult Cricket
                </Link>
            </div>
            <div className='flex-1 px2 mx-2'>
                <div className='flex justify-end'>
                <Link to='/' className='btn btn-ghost btn-sm rounded-btn'>
                        Home
                    </Link>
                    <Link to='/loadsaveddata' className='btn btn-ghost btn-sm rounded-btn'>
                        Load
                    </Link>
                    <Link to='/about' className='btn btn-ghost btn-sm rounded-btn'>
                        About
                    </Link>
                </div>
            </div>
        </div>
    </nav>
}

Navbar.defaultProps = {
    title: 'Catapult Cricket'
}

Navbar.PropsType = {
    title: PropsType.string,
}
export default Navbar